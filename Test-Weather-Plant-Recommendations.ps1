# ===============================================================
# Weather-Integrated Plant Recommendation Test 🌤️🌱
# نظام توصية النباتات مع التكامل الذكي للطقس والموسم
# ===============================================================

$BASE = "http://localhost:8081"

Write-Host "========================================" -ForegroundColor Blue
Write-Host "  🌤️ WEATHER-SMART PLANT RECOMMENDATIONS" -ForegroundColor Blue
Write-Host "========================================" -ForegroundColor Blue
Write-Host ""

# =============================================================================
# Step 1: Admin Setup with Season Support
# =============================================================================
Write-Host "--- Step 1: Admin Setup (with Season Question) ---" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "✅ Questions initialized (including seasonal support)!" -ForegroundColor Green
} catch {
    Write-Host "⚠️ Questions may already exist" -ForegroundColor Yellow
}

# =============================================================================
# Step 2: Check Current Weather/Season
# =============================================================================  
Write-Host "`n--- Step 2: Check Current Weather/Season ---" -ForegroundColor Yellow

try {
    # Get weather (this will be used automatically by recommendation system)
    $weather = Invoke-RestMethod -Uri "$BASE/api/user/home/weather" -Headers $adminHeaders
    Write-Host "🌤️ Current Weather:" -ForegroundColor Cyan
    Write-Host "   Season: $($weather.seasonAr) / $($weather.seasonEn)" -ForegroundColor White
    Write-Host "   Temperature: $($weather.temperature)°C" -ForegroundColor White
    Write-Host "   Description: $($weather.descriptionAr)" -ForegroundColor White
    if ($weather.isEstimated) {
        Write-Host "   ⚠️ Using estimated weather data (no API key configured)" -ForegroundColor DarkYellow
    } else {
        Write-Host "   ✅ Real weather data from API" -ForegroundColor Green
    }
} catch {
    Write-Host "⚠️ Could not fetch weather, using defaults" -ForegroundColor Yellow
}

# =============================================================================
# Step 3: User Login 
# =============================================================================
Write-Host "`n--- Step 3: User Login ---" -ForegroundColor Yellow
...
# =============================================================================
# Step 4: Get Questions (Now Including Season)
# =============================================================================
Write-Host "`n--- Step 4: Get Questions ---" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "`n=== NEW QUESTIONS WITH SEASON SUPPORT ===" -ForegroundColor Magenta
    foreach ($q in $questionsResponse.requiredQuestions) {
        Write-Host "Q$($q.questionId): $($q.questionTextAr)" -ForegroundColor Green
        if ($q.questionTextAr -like "*موسم*") {
            Write-Host "   🌤️ SMART SEASON QUESTION!" -ForegroundColor Yellow
        }
        foreach ($opt in $q.options) {
            $marker = if ($opt.optionTextAr -like "*الحالي*") { "🎯" } else { "  " }
            Write-Host "$marker [$($opt.optionId)] $($opt.optionTextAr)" -ForegroundColor White
        }
        Write-Host ""
    }
} catch {
...
# =============================================================================
# Step 5: Submit Smart Answers (Season Auto-Selected) 
# =============================================================================
Write-Host "--- Step 5: Submit Smart Answers ---" -ForegroundColor Yellow

# Build smart answers including "current season" selection
$smartAnswers = @()
...
foreach ($question in $questionsResponse.requiredQuestions) {
    if ($question.options.Count -gt 0) {
        ...
        if ($question.questionTextAr -like "*موسم*") {
            # للموسم: اختار "الموسم الحالي" ليتم تحديده تلقائياً من الطقس 
            $currentSeasonOpt = $question.options | Where-Object { $_.optionTextAr -like "*الحالي*" }
            $selectedOption = if ($currentSeasonOpt) { $currentSeasonOpt.optionId } else { $question.options[0].optionId }
            Write-Host "🌤️ Season: Auto-selecting 'Current Season' → Weather API will determine actual season" -ForegroundColor Yellow
        } else {
            # للأسئلة الأخرى: اختار أول خيار
...
Write-Host "`n🚀 Submitting WEATHER-SMART answers..." -ForegroundColor Cyan
Write-Host "The system will:" -ForegroundColor Yellow
Write-Host "  1. Get current weather/season automatically 🌤️" -ForegroundColor White
...
try {
    ...
    Write-Host "  🌤️ WEATHER-SMART RESULTS!" -ForegroundColor Blue
    Write-Host "========================================" -ForegroundColor Blue
    ...
    Write-Host "$($recommendationsResponse.summaryAr)" -ForegroundColor Cyan
    ...
    if ($recommendationsResponse.userConditionsSummary) {
        ...
        foreach ($condition in $recommendationsResponse.userConditionsSummary) {
            $marker = if ($condition -like "*🌤️*") { "🌤️" } else { "📋" }
            Write-Host "$marker $condition" -ForegroundColor White
        }
    }
    
    if ($recommendationsResponse.totalRecommendations -gt 0) {
        Write-Host "`n🌱 SEASONAL RECOMMENDATIONS:" -ForegroundColor Green
        foreach ($rec in $recommendationsResponse.recommendations) {
            $seasonalMarker = if ($rec.matchPercentage -gt 70) { "⭐" } else { "🌿" }
        ...
        try {
            ...
            Write-Host "   Match: $($details.matchPercentage)% ($($details.matchLevelAr))" -ForegroundColor Cyan
        } catch {
            Write-Host "⚠️ Could not get details" -ForegroundColor Yellow
        }
        
    } else {
        Write-Host "`n❌ No recommendations found!" -ForegroundColor Red
    ...
    Write-Host "`n❌ Failed to get weather-smart recommendations!" -ForegroundColor Red
